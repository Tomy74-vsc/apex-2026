...
- Default trading mode to paper trading with conservative risk parameters unless the user explicitly switches to live trading; every external call (RPC, Jupiter, HTTP) must have an explicit timeout and must not block indefinitely.
...
- The project APEX-2026 is a Solana HFT bot structured around MarketScanner, PumpScanner, Guard, DecisionCore, and Sniper executors. In curve-prediction mode, MarketScanner is disabled to save RPC quota. Cursor conventions live in `.cursor/rules/` as domain MDC files instead of legacy `.cursorrules`.
...
- FeatureStore (bun:sqlite) is append-only with 5s buffer flush, storing feature snapshots, token outcomes, curve_snapshots, curve_outcomes (`labelCurveOutcome` on `graduated` and real `evicted`; **not** on `evicted`+`reason===graduated` — internal cleanup only), optional whale wallet rows when enabled, model params for ML training, and export helpers including labeled curve rows for supervised CSV (`bun run export:ml`, optional `EXPORT_ML_LAST_SNAPSHOT_ONLY` / `--last-snapshot-per-mint`).
- Curve pipeline: `STRATEGY_MODE=curve-prediction` starts CurveTracker (TieredMonitor, BatchPoller) before PumpScanner so `registerNewCurve` never hits a null tiered monitor. `TieredMonitor` emits `syntheticTrade` on bonding-curve reserve deltas (`synthetic: true`) into `CurveTracker.recordTrade` for velocity; `BotDetector` and `WalletScorer` ignore synthetic rows for wallet-level heuristics. Entry gating is `EntryFilter.ts` (velocity first window, trivial-tx ratio, optional `ENTRY_GATE_SOL_FLOOR` implied SOL/min). Bonding-curve MTM: `calcPricePerToken` in `curve-math.ts`; `PositionManager` normalizes legacy prices on restore (SQLite v2). Paper curve uses `CurveExecutor`, `ExitEngine`, `GraduationExitStrategy`, `CurveShadowAgent` after `decideCurve` (legacy `ShadowAgent` stays on `processToken` only).
...
- **Stratégie quant (source de vérité)** : `APEX_QUANT_STRATEGY.md` — **§0** tranche sortie **Rotation** vs Conservation et **poids sociaux fixes** ; Marino arXiv:2602.14860, EV snipe T=0 négatif, **sweet spot ~35–55 SOL réel**, breakeven dynamique, **7 signaux + vétos V1–V5**, `safety_margin(confidence)`, Kelly fractionnel + caps, cascade §8 (hard/soft time, stall, regression progress, TP partiel ; **trailing 15 % sur reliquat post-TP 50 %** encore ouvert côté code).
...
- **Prochaine vague (Phase C / `raodmap_final`)** : pas encore dans le dépôt — **pas de** `GrokXScanner` / `NarrativeRadar` (grep) ; **WebSocketPool** ; bootstrap **WhaleWalletDB** / discovery ; **TelegramPulse** production + **DexScreenerMonitor** dédié ; couche ML export / orchestration comme dans roadmapv4 §P3. **Conflit guérilla vs doc** : APEX/roadmapv4 supposent **Grok/xAI** (payant ou crédits) — le cœur bot doit rester **free-tier** sauf opt-in explicite (`XAI_API_KEY`, etc.).
...
- **Done:** `TrackedCurve` lifecycle fields; `TieredMonitor` aggressive evictions + 60s sweep + **no evict** if open position; `BreakevenCurve.safetyMarginFromConfidence` + `calcBreakevenWithConfidence` (APEX §6, pas seulement ×1.5 fixe roadmapv3); `GraduationPredictor` **7 poids APEX §5**, **vétos V1→V5 dans l’ordre doc** puis gates roadmapv3 (`min_trades`, `early_hot`, `velocity_ratio`); **holder quality** = `(1 − freshWalletRatio) × (1 − top10Conc)` où `top10Conc` = **top-10 supply on-chain** si `HOLDER_DISTRIBUTION_ENABLED` + cache `HolderDistribution.ts`, sinon proxy **top10BuyVolumeShare** (volume achats); bandes `CURVE_ENTRY_*` **partagées** `src/constants/curve-entry-bands.ts` (Guard + prédicteur); heuristique **0 trade** : `confidence=0.35`, bande `CURVE_ENTRY_*` + veto **V5** stale age; `getVetoStats()`; `EntryFilter.ts` → `DecisionCore`; `AIBrain` Kelly `b=M−1`, **`MAX_POSITION_SOL`** cap (APEX §7), `MIN_KELLY_FRACTION` skip, `curvePredictionPGrad`; `ExitEngine` defaults **APEX §0.2 Rotation** + **`TRAILING_REMAINDER_PCT`** sur reliquat après TP 50 % (`PositionManager.markPartialTakeProfit`); `app` cache pGrad + dashboard; **outcomes** : `evicted` avec `reason===graduated` ne réécrit plus le label; **whale** auto `ensureObservedBuyer` sur résolution courbe (seuils env); **social** : `SOCIAL_BLEND_WEIGHT_*`, `NARRATIVE_SOCIAL_MODE`.
...
17. `HolderDistribution.ts` (**done** — optionnel RPC `HOLDER_DISTRIBUTION_ENABLED`), `StallDetector.ts` (if not fully subsumed by ExitEngine), `PortfolioGuard.ts`, curve `JitoBundler.ts`, `SniperV3`/`JitoTipOracle`, Drift — by priority after data validates edge.
