...
- Default trading mode to paper trading with conservative risk parameters unless the user explicitly switches to live trading; every external call (RPC, Jupiter, HTTP) must have an explicit timeout and must not block indefinitely. In curve-prediction, legacy-style filters (`MIN_LIQUIDITY`, full `analyzeToken` honeypot and Raydium checks) are not applied on entries unless live trading is enabled with `CURVE_FULL_GUARD=1`; `CurveTokenAnalyzer` skips `analyzeToken` unless `CURVE_FULL_GUARD=1` (stub security). Paper curve entries rely on `validateCurve`, blocking `CurveTokenAnalyzer` (wait up to `CURVE_ANALYZER_WAIT_MS` on cache miss), `EntryFilter`, and `GraduationPredictor` gates.
...
- Guard.ts centralizes legacy token safety via `analyzeToken` / `validateToken` (Promise.allSettled, RPC racing, top-10 holders, Jupiter honeypot, Raydium liquidity and LP-burn, timeouts). Mint decoding tries Token-2022 then legacy SPL; Jupiter honeypot simulation treats ambiguous TX build errors (e.g. sanitize accounts) as inconclusive, not a confirmed honeypot. For curve-prediction entries, `validateCurveForExecution` always applies synchronous `validateCurve` (progress band, max positions, age); it calls full `analyzeToken` only when `CURVE_FULL_GUARD=1` and `TRADING_MODE=live` with `TRADING_ENABLED=true`.
...
- `DecisionCore` courbe : `Guard.validateCurveForExecution` → `CurveTokenAnalyzer` (verdict ENTER requis; cache ou attente `analyze` jusqu’à `CURVE_ANALYZER_WAIT_MS`, SKIP si timeout) → `EntryFilter` → `AIBrain.decideCurve` (Kelly seul autorité taille) + `PortfolioGuard` avant `ENTER_CURVE`.
...
- Curve pipeline: `STRATEGY_MODE=curve-prediction` (seul mode supporté au boot) starts CurveTracker (TieredMonitor, BatchPoller) before PumpScanner so `registerNewCurve` never hits a null tiered monitor. `TieredMonitor` emits `syntheticTrade` on bonding-curve reserve deltas (`synthetic: true`) into `CurveTracker.recordTrade`; wallet-level heuristics ignore synthetic rows; velocity uses wallet-first metrics with optional `VELOCITY_FALLBACK_SYNTHETIC` and mixed columns in ML snapshots. New curve registration is gated by `MIN_CURVE_REGISTRATION_SOL` in PumpScanner (not `MIN_LIQUIDITY`, which is legacy Raydium). Entry gating is `EntryFilter.ts` (velocity first window, trivial-tx on non-synthetic buys, optional `ENTRY_GATE_SOL_FLOOR`). Holder concentration in `GraduationPredictor` uses on-chain top-10 supply when `HOLDER_DISTRIBUTION_ENABLED` and cache is warm, else a volume proxy; heuristique 0 trade wallet : `predictFromCurveState` avec `confidence=0.20` (pas le gate `MIN_TRADE_COUNT`). `CurveTokenAnalyzer` pré-chauffe HOT (cache 5 min) et enrichit le prédicteur via `fullAnalysis`; absence de couche security complète ne produit plus un véto global type `V0_security_missing`—seuls honeypot / risk explicites dans la couche security bloquent. `appendHotObservationSnapshot` (every HOT poll) stores predictor features for ML without EntryFilter/Guard/cooldown; trading decisions still flow through `processCurveEvent`. `BreakevenCurve` uses `SAFETY_MARGIN_BASE` default 1.5 when env unset. Bonding-curve MTM: `calcPricePerToken`; `PositionManager` normalizes legacy prices on restore (SQLite v2+). Paper curve uses `CurveExecutor`, `ExitEngine` (`PROGRESS_DROP_VETO`, hard max hold), `GraduationExitStrategy`, `PortfolioGuard`, `CurveShadowAgent` after `decideCurve`. `ShadowAgent` legacy reste chargé pour stats dashboard uniquement.
