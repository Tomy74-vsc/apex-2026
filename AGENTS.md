...
- FeatureStore (bun:sqlite) is append-only with 5s buffer flush, storing feature snapshots, token outcomes, curve_snapshots, curve_outcomes (labelCurveOutcome on graduated/evicted), optional whale wallet rows when enabled, and model params for ML training.
- Curve pipeline: entry gating uses `src/modules/entry/EntryFilter.ts` (velocity / trivial-tx gates) wired from `DecisionCore`, not under `src/ai/`. Bonding-curve mark-to-market: `src/math/curve-math.ts` `calcPricePerToken` returns SOL per minimal token unit (virtual SOL lamports / virtual token raw ÷ 1e9). `PositionManager` divides legacy lamports-per-raw prices (>1e-8) by 1e9 on recompute / restore; SQLite open-position payload version 2. Paper curve mode wires `CurveTracker`, `TieredMonitor`, and `BatchPoller` with `PositionManager`, `ExitEngine`, `GraduationExitStrategy`, and paper `CurveExecutor` for buys/sells.
- Social intelligence scaffolding lives in `src/social/` (`GrokXScanner.ts`, `NarrativeRadar.ts`, `SentimentAggregator.ts`), `src/ingestors/SocialTrendScanner.ts`, and `src/data/WhaleWalletDB.ts`; full production wiring and optional providers (e.g. xAI) depend on env and app boot paths.
