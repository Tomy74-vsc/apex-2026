...
- FeatureStore (bun:sqlite) is append-only with 5s buffer flush, storing feature snapshots, token outcomes, curve_snapshots, curve_outcomes (labelCurveOutcome on graduated/evicted), optional whale wallet rows when enabled, model params for ML training, and export helpers including labeled curve rows for supervised CSV (`bun run export:ml`).
